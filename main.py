...
def screen_global_markets(all_games_data):
    """
    Screens global markets. Filters out futures or far-off games by forcing a 48-hour window.
    """
    detected_legs = []
    ...
    for game in all_games_data:
        # TIME FILTER BLOCK: Stop futures/outrights from ruining the board
        commence_time_str = game.get("commence_time")
        if commence_time_str:
            try:
                # Parse the ISO timestamp from the API (ends in Z for UTC)
                game_time = datetime.strptime(commence_time_str, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
                
                # If the game is in the past, or further out than 48 hours, SKIP IT.
                if game_time < now_utc or game_time > max_window:
                    continue
            except Exception:
                pass # If time parsing glitches, keep moving safely
                ...
                for market in book.get("markets", []):
                    if market.get("key") == "h2h":
                        ...
                        for outcome in outcomes:
                            ...
                            # Standard game line screening thresholds
    ...
    # Sort so closest value variance floats to the top
    ...
    if not active_legs:
        return {
            ...
            "home_run_tickets": [{
                ...
                "legs": [{"sport": "📡", "trigger": "BOARD CLEAR", "bet": "No upcoming games match strict sharp parameters right now."}]
            }],
            "grind_tickets": [{"game": "Live Window Empty", "total_odds": "Even", "legs": ["Awaiting next live game board rotation."]}]
        }

    # Ticket 1: The High-End Home Run Combo (Top 3 games starting within 48 hours)
...
def main():
    ...
    for league in active_leagues:
        # Ignore outright/futures leagues completely to save API space
